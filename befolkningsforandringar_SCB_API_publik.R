...
# Sen för att använda är det bara att köra hela skriptet och kika i utskriftsmappen där det ska finnas ett antal diagram och två excelfiler
...
######## Inställningar ###########################################################################################################################
geo <- c("1281") # ändra geografi som ska användas här. Helsingborg är exempelvis 1283.                                                        ###
                                                                                                                                               ###
...
### Läser in färger som används längre ner i skriptet. Här går det att ändra till vilka färger man vill
farger <- rev(c("#76232F", "#E35205", "#F0B323",  "#C0843D", "#C5B9AC", "lightgrey"))

### Drar hem statistik från SCB över befolkningsförändringar på månadsbasis, fram till december 2024
pxweb_query_list <- 
  list("Region"=geo,
...
### Drar hem statistik från SCB över befolkningsförändringar på månadsbasis från januari 2025 och framåt
pxweb_query_list <- 
  list("Region"=geo,
       "Forandringar"=c("100","110","115","130","135","140","150","155","160","170","175","179","180","190","220","230","235","240","250","260","270"),
       "Kon"=c("TotSa"),
...
px_data <- 
  pxweb_get(url = "https://api.scb.se/OV0104/v1/doris/sv/ssd/BE/BE0101/BE0101G/MBefStatRegionCKM",
            query = pxweb_query_list)
px_data_frame2 <- as.data.frame(px_data, column.name.type = "text", variable.value.type = "text")

px_data_frame2 <- px_data_frame2 |> 
                            mutate(kön = "totalt")

# Slår ihop tabellerna från SCB
px_data_frame <- rbind(px_data_frame, px_data_frame2)
...
flyttnetto <- ggplot(flyttnetto_df, aes(fill = Typ, y=Antal, x=år)) + 
                          ...
                          geom_hline(yintercept=0, linetype="solid", color = "black", size=0.65)+
                          theme(panel.border = element_blank(),
                                axis.line = element_line(color = 'black'),
                                legend.position = "bottom",
                                panel.background = element_rect(fill = "white"),
